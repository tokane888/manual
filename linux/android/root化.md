...
* Magisk
  * 設定 => Magiskアプリを隠す => 戻る => 画面下の縦アイコン押下 => MagiskHide
    * 下記に対してMagiskHide有効化
      ...
      * kyash
      * Okta Verify
  * これでもsuicaが認識されず、タッチしても音がならず決済されないことがある
    * Magisk hideの有効／無効を切り替えると何故か直った
* アルテ日本語入力キーボード
  * root化関係ないが。。
...
* suicaが不安定
  * タッチ時に音がしない
  * Magisk hideの有効／無効を切り替えると何故か直る
  * SafetyNetが回避できていないので、回避で改善する可能性はある
  * 関連する可能性がゼロではないとのことなので、下記実施の上様子見中
    * USBデバッグ無効化
    * "Magiskアプリを隠す"有効化
